            ...
            try
            {
                ...
                switch (strPage)
                {
                    ...
                    case "B2016Page1": case "B2016Page2": case "B2016Page3":
                        ...
                        strQuery = "select b.bk_code from taxp_profile t, bank b where tp_5=? and t.tp_bank=b.bk_name";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        ...
                        //weihong 
                        strQuery = "select FORMAT(TP_WORKER_APPROVEDATE,'dd/mm/yyyy'), TP_COM_ADD_STATUS,FORMAT(TP_DOB,'dd/MMM/yyyy'),FORMAT(TP_HW_DOB,'dd/MMM/yyyy'),TP_GST,TP_TEL_KOD,TP_MOBILE_KOD,TP_FAX_KOD,TP_MOTHER_IC,TP_FATHER_IC,TP_BWA,TP_DOB,TP_BUSINESS_ECOMMERCE,TP_JKDM,TP_DISPOSAL1976,TP_LDMN_DISPOSAL from TAXP_PROFILE2 where TP_REF_NO=?";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer.ToString()));
                        daOdbc = new OdbcDataAdapter(cmdOdbc);
                    ...
                    //break;
                        ...
                        if (strPNLKey == "")
                        {
                         ...
                         //  bc_bus_entity, bc_code, bc_company,bc_type''
                        ...
                        else
                        {
                            ...
                            strQuery = "select bc_bus_entity, bc_code, bc_company,bc_type from business_source where bc_key = ? and bc_ya = ?  and bc_businesssource = (select pl_main_business from profit_loss_account where pl_ref_no = ? and pl_ya = ? and pl_maincompany = '1' order by pl_key)";
                            cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                            cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer));
                            cmdOdbc.Parameters.Add(new OdbcParameter("@ya", strYA));
                            cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer));
                        ...
                        strQuery = "select sum([exa_amount]) from [pl_expenses] where [exa_key] = ? and exa_pltype = 54";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        cmdOdbc.Parameters.Add(new OdbcParameter("@PNLKey", strPNLKey));
                        //cmdOdbc.Parameters.Add(new OdbcParameter("@ya", strYA));
                        daOdbc = new OdbcDataAdapter(cmdOdbc);
                        ...
                        //Business Loss CF
                        strQuery = "select sum(tca_balance_cf) as loss_cf from tax_adjusted_loss where tc_key=(select tc_key from tax_computation where tc_ref_no=? and tc_ya=?)";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer.ToString()));
                        cmdOdbc.Parameters.Add(new OdbcParameter("@ya", strYA));
                        daOdbc = new OdbcDataAdapter(cmdOdbc);
