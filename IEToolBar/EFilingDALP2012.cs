...
namespace IEToolBar
{
    public class EFilingDALP2012: EFilingDALP
    {
        public EFilingDALP2012()
        {
        }


        public EFilingDALP2012(string strTaxPayer, string strYA, string strTaxAgent)
        {
            //LEESH FEB 2012
        ...
        public DataSet GetFormDataP2012(string strPage)
        {
            //LEESH FEB 2012
            string strQuery = "";
            ...
            try
            {
                ...
                switch (strPage)
                {
                    ...
                    #endregion
                    #region "Page2"
                    case "P2012Page2": //Pendapatan Perniagaan
                        ...
                        #endregion
                    #region "Page3"
                    case "P2012Page3": //Pendapatan Perniagaan
                        strQuery = "SELECT [P_KEY] FROM [PARTNERSHIP_INCOME] WHERE [P_REF_NO]=?  AND P_YA=? ";//LeeCC Partnership
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer));
                        cmdOdbc.Parameters.Add(new OdbcParameter("@ya", strYA));
                    ...
                    case "P2012Page6":
                        ...
                        strQuery = "SELECT P_CP30_ASAL,format(P_CP30_ASAL_DATE,'dd/mm/yyyy'), P_CP30_PINDAAN, format(P_CP30_PINDAAN_DATE,'dd/mm/yyyy') FROM [PARTNERSHIP_INCOME]  where [P_KEY]=?";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                    ...
                    case "P2012Page8":
                        ...
                        strQuery = "SELECT PPL_SALES,PPL_OP_STK,PPL_PURCHASES_COST,PPL_CLS_STK, PPL_COGS," +
                            "PPL_OTH_BSIN,PPL_OTH_IN_DIVIDEND,PPL_OTH_IN_INTEREST,PPL_OTH_IN_RENTAL_ROYALTY,cdbl(PPL_OTH_IN_OTHER)+cdbl(PPL_NONTAX_IN_TOTAL)," +
                            "PPL_EXP_LOANINTEREST,PPL_EXP_SALARY,PPL_EXP_RENTAL,PPL_EXP_CONTRACT,PPL_EXP_COMMISSION," +
                            "PPL_BAD_DEBTS,PPL_TRAVEL,PPL_EXP_REPAIR_MAINT,PPL_EXP_PRO_ADV, cdbl(PPL_OTHER_EXP) + cdbl(PPL_LAWYER_COST) + cdbl(PPL_EXP_INT_RES), " +
